...
uint16_t Afant_ILI9341::color565(uint8_t r, uint8_t g, uint8_t b) {
    return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3);
}

Afant_ILI9341::Afant_ILI9341(int8_t cs, int8_t dc, int8_t mosi,
...
Afant_ILI9341::Afant_ILI9341(int8_t cs, int8_t dc, int8_t rst) : Adafruit_GFX(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT) {
    _cs   = cs;
    _dc   = dc;
    _rst  = rst;
    _sclk  = -1;
...
#ifdef ESP32
void Afant_ILI9341::begin(uint32_t freq, SPIClass &spi)
#else
void Afant_ILI9341::begin(uint32_t freq)
#endif
{
#ifdef ESP32
    _spi = spi;
...
void Afant_ILI9341::setRotation(uint8_t m) {
...
void Afant_ILI9341::invertDisplay(boolean i) {
    startWrite();
    writeCommand(i ? ILI9341_INVON : ILI9341_INVOFF);
    endWrite();
}

void Afant_ILI9341::scrollTo(uint16_t y) {
    startWrite();
...
void Afant_ILI9341::spiWrite(uint8_t b) {
    if(_sclk < 0){
        HSPI_WRITE(b);
        return;
    }
...
void Afant_ILI9341::startWrite(void){
    SPI_BEGIN_TRANSACTION();
    SPI_CS_LOW();
}

void Afant_ILI9341::endWrite(void){
    SPI_CS_HIGH();
    SPI_END_TRANSACTION();
}

void Afant_ILI9341::writeCommand(uint8_t cmd){
    SPI_DC_LOW();
    spiWrite(cmd);
    SPI_DC_HIGH();
}

void Afant_ILI9341::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
...
void Afant_ILI9341::pushColor(uint16_t color) {
  startWrite();
  SPI_WRITE16(color);
  endWrite();
}
...
void Afant_ILI9341::writePixel(uint16_t color){
    SPI_WRITE16(color);
}

void Afant_ILI9341::writePixels(uint16_t * colors, uint32_t len){
    SPI_WRITE_PIXELS((uint8_t*)colors , len * 2);
}

void Afant_ILI9341::writeColor(uint16_t color, uint32_t len){
#ifdef SPI_HAS_WRITE_PIXELS
    if(_sclk >= 0){
...
void Afant_ILI9341::writePixel(int16_t x, int16_t y, uint16_t color) {
    if((x < 0) ||(x >= _width) || (y < 0) || (y >= _height)) return;
    setAddrWindow(x,y,1,1);
    writePixel(color);
}

void Afant_ILI9341::writeFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color){
...
void Afant_ILI9341::writeFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color){
    writeFillRect(x, y, 1, h, color);
}

void Afant_ILI9341::writeFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color){
    writeFillRect(x, y, w, 1, color);
}

uint8_t Afant_ILI9341::readcommand8(uint8_t c, uint8_t index) {
    uint32_t freq = _freq;
    if(_freq > 24000000){
        _freq = 24000000;
    }
...
void Afant_ILI9341::drawPixel(int16_t x, int16_t y, uint16_t color){
    startWrite();
    writePixel(x, y, color);
    endWrite();
}

void Afant_ILI9341::drawFastVLine(int16_t x, int16_t y,
...
void Afant_ILI9341::drawFastHLine(int16_t x, int16_t y,
...
void Afant_ILI9341::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
        uint16_t color) {
    startWrite();
...
void Afant_ILI9341::drawRGBBitmap(int16_t x, int16_t y,
